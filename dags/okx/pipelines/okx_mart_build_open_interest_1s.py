...
DAG_ID = "okx_mart_build_open_interest_1s"
SCHEDULE = None
...
class EtlConfig:
    ...
    mart_table_fq: str = "okx_mart.agg_open_interest_1s"
...
SELECT
    ...
    first(open_interest, ts_event)                       AS oi_open,
    max(open_interest)                                   AS oi_high,
    min(open_interest)                                   AS oi_low,
    last(open_interest, ts_event)                        AS oi_close,
    avg(open_interest)                                   AS oi_avg,
    last(open_interest, ts_event)
      - first(open_interest, ts_event)                   AS oi_delta,

    first(open_interest_ccy, ts_event)                   AS oi_ccy_open,
    max(open_interest_ccy)                               AS oi_ccy_high,
    min(open_interest_ccy)                               AS oi_ccy_low,
    last(open_interest_ccy, ts_event)                    AS oi_ccy_close,
    avg(open_interest_ccy)                               AS oi_ccy_avg,
    last(open_interest_ccy, ts_event)
      - first(open_interest_ccy, ts_event)               AS oi_ccy_delta
...
with DAG(
    dag_id=DAG_ID,
    description="OKX MART: source-driven incremental catch-up rebuild for 1-second open interest aggregates",
    ...
    PythonOperator(
        task_id="build_open_interest_1s",
